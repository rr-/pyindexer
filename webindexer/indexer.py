...
def respond_not_found(request):
    ...
    response.text = jinja_env.get_template("not-found.htm").render(
...
def respond_access_denied(request):
    ...
    response.status = 403
    response.content_type = "text/html"
    response.text = jinja_env.get_template("access-denied.htm").render(
        path=request.path_info
    )
    return response
...
def respond_listing(request, local_path: Path, settings):
    ...
    response = Response()
    response.content_type = "text/html"
    response.text = jinja_env.get_template("index.htm").render(
        SortDir=SortDir,
        sort_styles=(
            (SortStyle.Name, "name"),
            (SortStyle.Size, "size"),
...
def catch_all_route(request):
    ...
    if not local_path.is_dir():
        ...
        return FileResponse(
            str(local_path), content_type=get_mimetype(local_path)
        )
